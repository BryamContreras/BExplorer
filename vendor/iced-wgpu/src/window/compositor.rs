    ...
    pub async fn request<W: compositor::Window>(
        ...
        let (format, alpha_mode) = compatible_surface
            ...
            .and_then(|surface| {
                ...
                // Some modern Wayland compositors advertise a 10-bit color
                // format first, even when no sRGB format is available.
                // `Rgb10a2Unorm` only has 2 bits of alpha, which turns a
                // transparency slider into four large opacity steps. Prefer
                // an 8-bit RGBA/BGRA fallback so translucent windows retain
                // the full 8-bit alpha precision used by older KWin versions.
                let is_8_bit_color = |format: &&wgpu::TextureFormat| {
                    matches!(
                        format,
                ...
                let format = formats
                    .iter()
                    .find(|format| {
                        is_8_bit_color(format)
                            && format.is_srgb() == color::GAMMA_CORRECTION
                    })
                    .copied()
                    .or_else(|| {
                        formats.iter().find(is_8_bit_color).copied()
                    })
                    .or_else(|| {
                        formats
                            .iter()
                    ...
                    .or_else(|| {
                        log::warn!("No preferred 8-bit surface format found!");
                ...
                // Iced's quad and triangle shaders premultiply fragment
                // colors and its pipelines use premultiplied alpha blending.
                // Prefer the matching surface mode when modern compositors
                // advertise both; interpreting this framebuffer as
                // postmultiplied causes washed-out colors and abrupt opacity
                // transitions on KWin/Wayland.
