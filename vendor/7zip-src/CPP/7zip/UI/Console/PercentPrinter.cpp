...
thread_local BfpProgressCb g_BfpProgressCb = NULL;
thread_local void *g_BfpProgressUserData = NULL;
...
{
  if (!_so) { _printedString.Empty(); return; }
...
{
  if (_bfpProgressCb && _tickStep != 0)
  {
    // Separate throttle so the hook still fires at a bounded rate even
    // when DisablePrint is true (which makes the main Print() return
    // before reaching _prevTick).
    const DWORD tick = GetTickCount();
    if ((UInt32)(tick - _bfpPrevTick) >= _tickStep)
    {
      _bfpPrevTick = tick;
      AString fileNameA;
      if (!FileName.IsEmpty())
        ConvertUnicodeToUTF8(FileName, fileNameA);
      _bfpProgressCb(Completed, Total, Files,
          Command.IsEmpty() ? NULL : (const char *)Command.Ptr(),
          FileName.IsEmpty() ? NULL : (const char *)fileNameA.Ptr(),
          _bfpProgressUserData);
    }
  }
  if (DisablePrint || !_so)
    return;
