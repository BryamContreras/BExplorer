...
impl Scrollbar {
    ...
    pub fn scroller_min_length(
        mut self,
        scroller_min_length: impl Into<Pixels>,
    ) -> Self {
        self.scroller_min_length = scroller_min_length.into().0.max(0.0);
        self
    }
    ...
    fn new(
        ...
        let y_scrollbar = if let Some(vertical) = show_scrollbar_y {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                // Keep a configurable minimum height for easier grabbing with
                // super tall content.
                let scroller_height = (scrollbar_bounds.height * ratio)
                    .max(scroller_min_length)
                    .min(scrollbar_bounds.height);
                let scrollable_height = content_bounds.height - bounds.height;
                let scroll_percentage = if scrollable_height > 0.0 {
                    (translation.y / scrollable_height).clamp(0.0, 1.0)
                } else {
                    0.0
                };
                let scroller_offset =
                    scroll_percentage * (scrollbar_bounds.height - scroller_height);
        ...
        let x_scrollbar = if let Some(horizontal) = show_scrollbar_x {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                // Keep a configurable minimum width for easier grabbing with
                // extra wide content.
                let scroller_length = (scrollbar_bounds.width * ratio)
                    .max(scroller_min_length)
                    .min(scrollbar_bounds.width);
                let scrollable_width = content_bounds.width - bounds.width;
                let scroll_percentage = if scrollable_width > 0.0 {
                    (translation.x / scrollable_width).clamp(0.0, 1.0)
                } else {
                    0.0
                };
                let scroller_offset =
                    scroll_percentage * (scrollbar_bounds.width - scroller_length);
