    ...
    fn draw(
        ...
        for (index, span) in self.spans.as_ref().as_ref().iter().enumerate() {
            ...
            if span.highlight.is_some()
                ...
                || is_hovered_link
            {
                // `text::draw` anchors the paragraph inside the widget bounds.
                // Span decorations must use that same origin or their
                // backgrounds drift away from centered, multi-line text.
